...
function cleanSpec(object) {
    ...
    for (const [key, value] of Object.entries(object)){
        ...
        if (key === "$ref") {
            return {
                $ref: value.replace("#", "#/components/schemas/")
            };
        }
        // If is a object
        if (typeof value === "object" && !Array.isArray(value)) {
            ...
            if (Array.isArray(cleanedValue.type)) {
                cleanedValue.anyOf = cleanedValue.type.map((t)=>({
                        type: t
                    }));
    ...
    for (const route of apiRoutes){
        ...
        for (const method of methods){
            ...
            spec.paths[path][method.toLowerCase()] = {
                ...
                requestBody: "body" in schema && [
                    "PUT",
                    "PATCH",
                    "POST"
                ].includes(method) ? parsePayload(schema.body) : undefined
            };
        }
    }
